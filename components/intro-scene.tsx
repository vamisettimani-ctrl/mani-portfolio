...
export default function IntroScene({ onEnter }: { onEnter: () => void }) {
  return <section className="intro-scene" aria-label="Introduction">
    ...
    <div className="intro-visual"><div className="intro-photo-wrap"><img className="intro-photo" src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/professional%20pic-j0dnazyYUit8OzwMNiQT61Oicn8KvN.jpeg" alt="Mani Sai Sudheer in a professional suit" /><span>Profile / 01</span></div><div className="intro-canvas"><Canvas camera={{ position: [0, 0, 5], fov: 38 }} dpr={[1, 1.5]}><ambientLight intensity={1.5} /><pointLight position={[3, 3, 4]} intensity={20} color="#f9a8d4" /><Float speed={1.4} rotationIntensity={0.35} floatIntensity={0.5}><PinkWheel /><Text position={[0, -0.1, 0]} fontSize={0.34} color="#f8fafc" anchorX="center" anchorY="middle">MSS</Text></Float><OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.4} /></Canvas></div></div>
